/*
  ...
  异步任务:
  IO 操作太慢, CPU 处于闲置状态, CPU 可以不管IO 操作, 挂起处于等待中的任务, 先运行排在后面的任务, 等 IO 操作返回了
...
// 事件监听的优点: 解耦, 可以绑定多个事件, 每个事件可以指定多个回调函数
// 缺点: 整个程序变成了事件驱动型, 很难看出主流程
...
// 发布/订阅, 信号 信号中心 观察者模式
// 优点: 可以通过 "消息中心" 了解存在多少信号, 每个信号有多少订阅者, 从而监控程序的执行
$.subscribe('done', f2)
function f1() {
  setTimeout(() => {
    // ... 代码
    // f1 的代码执行完成之后, 向信号中心 $ 发布 done 信号, 从而引发 f2 的执行
    // f1 执行完毕, 发布信号, f2 订阅这个信号, 知道自己改执行了
    $.publish('done')
  }, 1000)
}
